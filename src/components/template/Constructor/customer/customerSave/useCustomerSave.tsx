import { useFormCustom } from "@/lib/form";
import { ROUTES } from "@/routes";
import { useCustomerDetailQuery } from "@/service/constructor/Customer/getDetail";
import { postCustomer, putCustomer } from "@/service/constructor/Customer/save";
import { RequestBody } from "@/service/constructor/Customer/save/type";
import { toastError, toastSuccess } from "@/toast";
import { useMutation } from "@tanstack/react-query";
import { useParams, useSearchParams } from "next/navigation";
import { useRouter } from "next/router";
import { useEffect, useMemo, useState } from "react";
const defaultValues = {
  code: "",
  name: "",
  description: "",
  isActive: true,
};

export default function CustomerSave() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const params = useParams();

  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);

  const actionType = searchParams.get("actionType");
  const isView = actionType === "VIEW";

  const id = Number(params?.id);
  const isUpdate = !!id;

  const methodForm = useFormCustom<RequestBody["SAVE"]>({
    defaultValues,
  });

  const { reset, handleSubmit, control } = methodForm;

  const { data, isLoading } = useCustomerDetailQuery(id, {
    enabled: !!id,
  });

  const { mutate } = useMutation({
    mutationFn: (body: RequestBody["SAVE"]) =>
      isUpdate ? putCustomer(id, body) : postCustomer(body),

    onSuccess: (res: any) => {
      toastSuccess("Thành công");
      if (res?.data?.id) {
        router.push(`${ROUTES.CUSTOMER}/${res.data.id}?actionType=VIEW`);
      }
    },

    onError: (error: any) => {
      toastError(error?.message || "Có lỗi xảy ra");
    },
  });

  const onSubmit = handleSubmit((data) => {
    mutate(data);
  });

  useEffect(() => {
    if (id && data?.data) {
      reset(data.data);
    }
  }, [id, data, reset]);

  return [
    { isView, page, rowsPerPage, control, isUpdate, isLoading, id },
    { setPage, setRowsPerPage, onSubmit },
  ];
}
